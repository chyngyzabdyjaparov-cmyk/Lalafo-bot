...
CHECK_INTERVAL = 7200  # 2 часа — чтобы уложиться в бесплатный лимит ScraperAPI (1000 запросов/мес)
SAMPLE_SIZE = 15
SEEN_FILE = "seen_ads.json"

SCRAPERAPI_KEY = os.environ.get("SCRAPERAPI_KEY", "214b99ce845ea149bdf9c21aeb5ca470")
...
def fetch_category_html():
    try:
        ...
        r = _session.get(SCRAPERAPI_URL, params=params, timeout=60)
        log.info("Запрос через ScraperAPI: статус %s, длина ответа %d символов", r.status_code, len(r.text))
        r.raise_for_status()
        return r.text
    except Exception as e:
        log.error("Ошибка запроса к Lalafo через ScraperAPI: %s", e)
        return None
